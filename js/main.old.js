...
const camera = new THREE.PerspectiveCamera(45, innerWidth/innerHeight, 0.1, 60000);
...
// sunDist — базовое (до умножения на distScale) расстояние объекта от Солнца,
// нужно для расчёта максимума движка масштаба (см. updateZoomMax)
const selectable = [
...
// самая дальняя орбита (Нептун) — радиус сферы, в которую вписана вся Солнечная система
const MAX_PLANET_DIST = Math.max(...PLANETS.map(p => p.dist));

// пересчитывает верхнюю границу движка масштаба так, чтобы с текущего центра
// камеры на максимуме зума в кадр гарантированно попадали Солнце и все планеты,
// независимо от того, какой объект сейчас выбран, и от текущего значения
// ползунка "Расстояние Земля–Солнце"
function updateZoomMax(target){
  const targetSunDist = (target && target.sunDist != null) ? target.sunDist : SUN_BASE_DIST;
  // худший случай: нужная планета находится на противоположном краю системы от цели
  ...
  const limitHalfFov = Math.min(vHalfFov, hHalfFov); // берём более узкую сторону кадра
  const margin = 1.15; // запас, чтобы планеты не упирались в самый край экрана
  ZOOM_MAX = Math.max(span * margin / Math.tan(limitHalfFov), controls.minDistance * 1.5);
...
const ui = initUI({
  ...
  onDistScaleChange(v){ distScale = v; updateZoomMax(focused); },
...
updateZoomMax(focused); // начальный расчёт диапазона движка масштаба для Земли
...
// --- Астероид (issue #12: меш+хвост; движение/таймер — issue #13) ---
const asteroid = createAsteroid(0.5);
...
addEventListener('resize', () => {
  ...
  renderer.setSize(innerWidth, innerHeight);
  labelRenderer.setSize(innerWidth, innerHeight);
  updateZoomMax(focused); // при смене соотношения сторон меняется угол обзора, влияющий на предел
});
